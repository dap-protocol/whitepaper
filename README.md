...
Tier assignment is driven by cryptographically attested SLD counts — see §6.5.
...
3. **The transfer carrying the conversion pays the personal-class graduated burn.** Class changes take effect after the covenant applies, never during
4. **The transfer lock follows ownership duration, not class.** A converted TLD transferred within three years of `personal_since` pays the graduated burn brackets; at year 3+ transfers are free, as they would have been without converting
5. SLD usage requirements activate from the conversion height—the owner gets the same Year 1/Year 2 runway as a fresh registrant
6. Renewal fee drops to base rate on the next renewal cycle
7. Conversion is irreversible
...
### 6.5 SLD‑Count Attestation

Anti‑squat tiers depend on SLD counts, but SLD registration is deliberately off‑chain — TLD owners run any registry software they choose. Self‑reported counts would price fraud at zero; forcing SLDs on‑chain would destroy registry flexibility. Dap closes this gap with **anonymous counting credentials** and a **public transparency log**, keeping all heavy cryptography off‑consensus.

**How it works**:

1. Registrants hold an anonymous credential carrying a hidden registration counter, bound to a small locked on‑chain anchor (one credential per anchor — identities cost capital). The issuer publishes per‑epoch issuance transparency: live‑credential totals against a committed anchor set, so credential inflation is publicly detectable.
2. Each counted SLD registration produces a zero‑knowledge proof and a deterministic nullifier, appended to a public append‑only log. The same SLD can never be counted twice; one registrant’s contribution per TLD is capped; nobody learns who registered what.
3. An attester quorum independently replays the log and signs a compact **Tier Attestation** (TLD, tier, epoch, log checkpoint). Tier thresholds are age‑aware, keyed to the TLD’s on‑chain registration height. Consensus verifies plain ECDSA signatures inside RENEW/UPDATE — no zero‑knowledge verification ever touches the chain.
4. PENALIZE reads the attested tier from name state. Missing or stale attestations default to the unused tier: every failure mode over‑charges squatters, never under‑charges them.

**Trust split** (Certificate Transparency pattern): the credential issuer at `count.dap.sh` holds the pen; the log at `log.dap.sh` is everyone watching the pen; attesters sign what the log proves. Every attestation is publicly recomputable — a false one is non‑repudiable evidence of fraud.

**Tier thresholds require distinct registrants, not only SLD totals**:

| Tier    | Counted SLDs | Distinct registrants | Renewal        |
| ------- | ------------ | -------------------- | -------------- |
| Active  | 100+         | 50+                  | 1,000 GRP/yr   |
| Low‑use | 1+           | 10+                  | 10,000 GRP/yr  |
| Unused  | —            | —                    | 100,000 GRP/yr |

Denominating relief in identities closes the cliff where one fake SLD would have saved 90,000 GRP/year. We state the economics plainly: fakery is not impossible, it is bounded, priced per identity, and permanently recorded in a replayable public log. That is what self‑reporting and trusted oracles can never offer.

**Registration is never gated.** Counting is opt‑in per registration: registries — including free ones — serve anyone, with or without credentials. Credentials determine what counts toward the tier.

**Rollout** follows the standard sunset pattern: Foundation‑operated issuer and single attester at launch (with full public auditability from the start), a 3‑of‑5 federation by Year 1–3, and a threshold‑issued credential scheme with an open, bonded, slashable attester set by Year 3.
...
**No Special Treatment**: Legacy operators have no automatic claims, no DNSSEC proof shortcuts, no priority access. Dap is a level playing field. Your ICANN credentials mean nothing here; what matters is your willingness to build and your GRP to bid.
...
| Native browser integration                   | Ongoing  | Mass adoption (the goal)        |
...
1. Bid on `.google` in Dap’s auction (they should)
2. Ignore Dap (their choice)
...
The Dap owner of `.google` has the same rights as every other TLD owner. They must meet usage requirements or face reclaim. If they squat, they pay escalating fees. If they build, they thrive.
...
- Blake3 PoW prevents costless block production: computational work is required
...
**Peer scoring**: Each misbehavior adds points toward a ban threshold of 100. Reaching the threshold results in a 24‑hour ban. Examples: consensus‑invalid block or invalid‑PoW announcement (+50), invalid transaction (+20), rejected non‑duplicate transaction (+10), rate‑limit violations (TX +20, INV +15, expensive requests +10), unknown message type (+10), message‑handler error (+20). A separate 0‑1 quality score (starting at 0.5) tracks long‑term peer reliability; peers dropping below 0.1 are banned.
